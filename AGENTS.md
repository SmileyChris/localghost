...
- Keep the Compose project named `localhost` and the Docker network named
  `localhost-proxy`.
...
- Keep generated router and service names project-scoped, with explicit backend
  port and `localhost-proxy` network labels.
...
COMPOSE_PROJECT_NAME=localhost-fixture-a docker compose -f examples/compose.yaml config --quiet
