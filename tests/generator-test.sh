...
COMPOSE_PROJECT_NAME=generator-fixture uv run local-dev-proxy generate \
...
assert set(web["networks"]) == {"application", "local-dev-proxy"}
assert web["labels"]["traefik.enable"] == "true"
assert web["labels"]["traefik.docker.network"] == "local-dev-proxy"
...
assert "local-dev-proxy" not in model["services"]["worker"]["networks"]
PY
...
(
  ...
  COMPOSE_PROJECT_NAME=host-fixture uv run --frozen --project "${ROOT_DIR}" \
    local-dev-proxy generate --no-input --mode host --port 3000
)
...
assert set(app["networks"]) == {"local-dev-proxy"}
assert app["labels"][
...
(
  ...
  COMPOSE_PROJECT_NAME=dockerfile-fixture uv run --frozen --project "${ROOT_DIR}" \
    local-dev-proxy generate --no-input --mode dockerfile --port 80
)
...
assert set(app["networks"]) == {"default", "local-dev-proxy"}
assert app["labels"][
...
cp "${ROOT_DIR}/tests/fixtures/generator/compose.override.yaml" \
  "${EXTENDED_OVERRIDE}"
COMPOSE_PROJECT_NAME=generator-fixture uv run --frozen local-dev-proxy generate \
...
if COMPOSE_PROJECT_NAME=generator-fixture \
  uv run local-dev-proxy generate --no-input \
